...
int main() {
    ...
    // 복구 실행 및 시간 측정
    double recovery_time = measure_time(do_recovery_custom, kvs, "kvs.img");
    printf("Custom do_recovery time: %.6f seconds\n", recovery_time);
    ...
    // 스냅샷 생성 및 시간 측정
    double snapshot_time = measure_time(do_snapshot_custom, kvs, "kvs.img");
    printf("Custom do_snapshot time: %.6f seconds\n", snapshot_time);
